...
export async function generateReplyDraft(
  ...
  const toneInstructions: Record<string, string> = {
    'empathetic-direct': `Be concise and direct (3-5 sentences total). Acknowledge the frustration in ONE short sentence, then immediately pivot to the solution. Use "I built" to introduce the tool. Focus on what it does, not your personal story. End the reply with the raw URL on its own line so the reader can see and click it easily. Do NOT write a long empathy paragraph. Structure: one-line empathy → "I built [tool name]..." → 1-2 key features → raw URL on its own line.`,
    'shared-journey': `Open with a personal mini-story about YOUR specific experience hitting the same problem (e.g. "I switched from Kindle to Kobo last year and the first thing I noticed was..."). Be specific about what frustrated you — don't just say "I had the same problem", describe the actual scenario. Then naturally transition to "so I ended up building..." to introduce the tool. Be warm and conversational, slightly longer (5-7 sentences). End the reply with the raw URL on its own line. Structure: specific personal anecdote (2-3 sentences) → "so I built..." → features → raw URL on its own line → brief encouragement.`,
    'community-veteran': `Write like someone who has used this platform for a long time and is giving advice to a newer user. Do NOT open with "I" — start by validating that this is a known issue in the community (e.g. "The annotation export situation is definitely one of the biggest gaps..."). Then casually mention you made something to help, using a low-key tone ("I ended up making a simple browser tool for this"). Keep it brief (4-5 sentences). End with positive encouragement about the platform overall ("it really does grow on you", "hang in there", "once you get past this quirk..."). End the reply with the raw URL on its own line. Structure: community perspective on the issue → casual tool mention → raw URL on its own line → platform encouragement.`,
